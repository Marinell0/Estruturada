...
void redimensiona(FILE *entrada, int *atual, int tela){
	...
	if(negado){
		printf("Esta funcao ainda nao esta pronta mas a tela nao foi redimensionada\n");///////////////////////////////
	}else{
		for(i=0; i<=*atual; i++){
			if(x == posicoes[i].x && y == posicoes[i].y){
	...
	for(dx, dy; dx>=0 && dy>=0 ; dx--, dy--){
		...
		for(i=0; i<*posatual; i++){
			...
			if(posicoes[aba].x < 0 || posicoes[aba].y < 0 || (posicoes[aba].x + posicoes[aba].largura) > telas[tela].xmax || (posicoes[aba].y + posicoes[aba].altura) < telas[tela].ymax){
				...
				*dxerro = dx;
				*dyerro = dy;
				posicoes[aba].x = posicoes[aba].x + *dxerro;
				posicoes[aba].y = posicoes[aba].y + *dxerro;
				return;
			}
		...
		printf("Passei o pente x%d", dx);
			
	}
	
	printf("\nHorizontal: %d, vertical: %d\n", horizontal, vertical);
	
	posicoes[aba].x = posicoes[aba].x + horizontal;
	posicoes[aba].y = posicoes[aba].y + vertical;
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
			if(strcmp(comando, "MOVE")==0){
				...
				printf("dx erro e %d dy erro e %d\n", dxerro, dyerro);
