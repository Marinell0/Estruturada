...
void abre(FILE *entrada, int *atual, int xmax, int ymax, struct posicao *posicoes){
...
void fecha(FILE *entrada, int *atual, int xmax, int ymax, struct posicao *posicoes){
...
void redimensiona(FILE *entrada, int *atual, int xmax, int ymax, struct posicao *posicoes){
...
void move(int x, int y, int dx, int dy, int *posatual, int *dxerro, int *dyerro, struct posicao *posicoes, int xmax, int ymax){
	...
	for(dx=dx, dy=dy; dx>0 && dy>0 ; dx--, dy--){
		...
		for(i=0; i<=*posatual; i++){
			...
			(posicoes[aba].x >= posicoes[i].x && (posicoes[aba].x + posicoes[aba].largura) >= (posicoes[i].x + posicoes[i].largura) && posicoes[aba].y <= posicoes[i].y && (posicoes[aba].y + posicoes[aba].altura) >= (posicoes[i].y + posicoes[i].altura))){
				dx++;
				dy++;
				move(posicoes[i].x, posicoes[i].y, dx, dy, posatual, dxerro, dyerro, posicoes, xmax, ymax);
			}
		}
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
			if(strcmp(comando, "ABRE")==0){
				abre(entrada, &posatual, xmax, ymax, posicoes);
			}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
			if(strcmp(comando, "FECHA")==0){
				fecha(entrada, &posatual, xmax, ymax, posicoes);	
			}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
			if(strcmp(comando, "REDIMENSIONA")==0){
				redimensiona(entrada, &posatual, xmax, ymax, posicoes);
			}
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
			if(strcmp(comando, "MOVE")==0){
				int x, y, dx, dy, dxerro=0, dyerro=0;
				fscanf(entrada, "%d %d %d %d", &x, &y, &dx, &dy, xmax, ymax, posicoes);
				move(x, y, dx, dy, &posatual, &dxerro, &dyerro, posicoes, xmax, ymax);
